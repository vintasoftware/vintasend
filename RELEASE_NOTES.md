...
- New attachment manager seam: `BaseAttachmentManager` and `AsyncIOBaseAttachmentManager`
  ...
  notification backend never reads, writes, or downloads a file itself. Core ships the ABCs plus
  a working reference, `FakeAttachmentManager` / `FakeAsyncIOAttachmentManager`; real managers
  (local disk, S3, Django storage, and so on) live in their own `vintasend-*` package, the same
  way real backends and adapters do. See [ATTACHMENTS.md](ATTACHMENTS.md) for the full seam.
- Both services take a new `attachment_manager` constructor argument (instance, dotted import
  string, or the new `NOTIFICATION_ATTACHMENT_MANAGER` setting), and inject it into the backend
  through the duck-typed `inject_attachment_manager` hook. A backend that predates this seam and
  has no such method is left untouched and keeps working with no attachment support.
- The attachment model is now checksum-indexed: `AttachmentFileRecord` describes one stored blob,
  and `StoredAttachment` is the join row linking a notification to it (`StoredAttachment.file_id`
  points at the record; `StoredAttachment.id` remains the join row's own id).
  `storage_metadata` is kept as a deprecated alias for the new `storage_identifiers` field.
- New `NotificationAttachmentReference(file_id=...)` lets a caller attach an already-uploaded file
  by id instead of re-uploading it. `NotificationAttachment` (an upload) and
  `NotificationAttachmentReference` (a reference) are both accepted wherever attachments are
  passed in, through the new `AnyNotificationAttachment` union, distinguished with the new
  `is_attachment_reference` type guard.
- Identical uploads are deduplicated: attaching the same bytes twice reuses one
  `AttachmentFileRecord` (matched on checksum and size) and only adds a new join row. A reference
  to an unknown `file_id` raises the new `AttachmentFileNotFoundError`.
- New `get_orphaned_attachment_files` returns file records no longer referenced by any
  notification, for a caller-driven, two-step reclamation:
  `manager.delete_file_by_identifiers(...)` to delete the bytes, then
  `backend.delete_attachment_file(...)` to drop the row. Nothing is deleted automatically —
  cascading a join row when a notification is deleted is the backend schema's job, and running a
  periodic sweep is the host application's decision.
- New `AttachmentUploadError` and `UnsupportedAttachmentFileTypeError` exceptions
  (`vintasend.exceptions`), both deriving from `NotificationError`.

### Backwards compatibility
- **Seven new abstract methods were added to both `BaseNotificationBackend` and
  `AsyncIOBaseNotificationBackend`**: `store_attachment_file_record`,
  `get_attachment_file_record`, `find_attachment_file_by_checksum`, `delete_attachment_file`,
  `get_orphaned_attachment_files`, `get_attachments`, and `delete_notification_attachment`.
  **Every downstream backend subclass MUST implement all seven, on both classes, or it raises
  `TypeError` at instantiation.** This is the accepted cost of shipping the change as a minor
  release rather than a major one, following the same practice as version 1.2.0.
  `inject_attachment_manager` is added too, but concrete with a default implementation, so no
  existing backend needs to change for that one.
- **`vintasend-sqlalchemy` cannot adopt this release yet.** It is already missing methods added in
  1.2.0 (`persist_one_off_notification`, `mark_sent_as_read_bulk`, the in-app filter methods) and
  needs its own catch-up release before it can implement the seven new methods here.
- **`vintasend-django` must implement the new seam before it can pin `vintasend>=1.5.0`.** That
  implementation, plus a schema migration for the new `AttachmentFile` table, ships in its own PR
  in the `vintasend-django` repository after this release lands.
- **No data migration ships with this release, and none is needed.** Attachments on Django
  notifications never worked with the documented `NotificationAttachment` API — the old
  `_store_attachments` dispatched on attributes (`file_path`, `file_bytes`, `file_obj`) the
  dataclass has never had, so any real attachment raised `ValueError` before a row was ever
  written. There is no production data to carry forward.
- **`UpdateNotificationKwargs.attachments` intentionally stays typed `list[StoredAttachment]`**,
  not widened to `AnyNotificationAttachment`. `persist_notification_update` has no upload path —
  it only sets fields on an existing row — so accepting a raw upload or reference there would
  store it as if it were already a persisted attachment, which is wrong.
- No existing method signature or semantic changed outside the additions above; every other
  seam method, backend, adapter, and renderer needs no changes.

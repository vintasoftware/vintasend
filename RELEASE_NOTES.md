...
### Features
- `filter_notifications(filter, page, page_size, order_by=None)`, `count_notifications(filter)`
  and `get_backend_supported_filter_capabilities()` on `NotificationService` and
  `AsyncIONotificationService`. The composable filter vocabulary lives in the new
  `vintasend.services.notification_backends.filters` module: field filters (scalar equality or
  ...
  reports which fields, lookups and sort fields the configured backend supports, so a client (a
  monitoring dashboard, for example) can grey out what it can't use. See the README's "Filtering
  and Ordering Notifications" section for the full grammar and worked examples.
- `resend_notification(notification_id, use_stored_context_if_available=False)` on both services:
  ...
  original untouched. Refuses one-off notifications and notifications still scheduled in the
  future by raising `NotificationResendError`. `use_stored_context_if_available=True` reuses the
  source's stored context verbatim instead of regenerating it through the context registry.
...
### New exceptions
- `TenantReassignmentError`: raised by `update_notification` on both services when `tenant`
  appears in the update kwargs. `tenant` cannot be changed after a notification is created.
- `NotificationResendError`: raised by `resend_notification` for a one-off notification or one
...
- **`filter_notifications` is a newly added `@abstractmethod` on both `BaseNotificationBackend`
  ...
  it can be instantiated against 1.5.0. `get_filter_capabilities` (default `{}`, meaning every
  capability is supported) and `count_notifications` (default: pages through
  `filter_notifications` and sums the results) are concrete, so they need no changes but SHOULD be
  overridden for efficiency (e.g. a database `COUNT`). `vintasend-django` will release its own
  filter implementation after this repo's release lands; `vintasend-sqlalchemy` cannot adopt it
  until its own catch-up plan lands, since it is already missing four methods from 1.2.0.
- The `sent_at`, `read_at` and `tenant` additions to `Notification` / `OneOffNotification` are
  additive fields with `None` defaults, appended after the existing ones -- existing positional
  construction and existing callers are unaffected. `persist_notification` and
  `persist_one_off_notification` gained an optional trailing `tenant` keyword; the service forwards
  it only when the caller actually passes one, so a backend built against a pre-1.5.0 signature
  keeps working for tenant-less callers and only raises `TypeError` if a caller passes `tenant` to
  it.
- `update_notification` on both services now raises `TenantReassignmentError` if `tenant` is
  present in the update kwargs, checked on the raw dict rather than only the typed shape. An
  application that was already passing `tenant` to `update_notification` (there was previously no
  supported way to set it there, since `tenant` is only accepted by the create and persist paths)
  will now get an exception instead of a silently ignored value.
- `send()` gained an optional trailing `context` keyword, used internally by
  `resend_notification` to bypass context regeneration. Every existing caller passes no `context`
  and is unaffected.
- No existing seam method was renamed, reordered or removed.

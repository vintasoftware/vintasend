...
class BaseNotificationReplicationQueueService(ABC):
    """
    Base class for notification replication queue services. All replication queue services
    ...
    This is the sibling of ``BaseNotificationQueueService``: the send queue hands a worker a
    notification id to deliver, while the replication queue hands a worker a
    ...
    `NotificationError` subclass. `NotificationService._execute_multi_backend_write` calls
    `enqueue_replication` per additional backend in queued mode and does not expect a raw broker
    exception to escape: a failed enqueue is caught and that one backend is replicated inline
    instead, so a broken queue never silently drops replication.

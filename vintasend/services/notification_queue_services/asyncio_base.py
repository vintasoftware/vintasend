...
class AsyncIOBaseNotificationQueueService(ABC):
    ...
    `NotificationError` subclass. `AsyncIONotificationService.send()` calls
    `enqueue_notification` inside its adapter loop and does not expect a raw broker exception
    (for example `kombu.exceptions.OperationalError`) to escape. An unwrapped exception would
    propagate through `send()` regardless of `raise_on_failed_send`, breaking that setting's
    contract.
    ...
    `send()` awaits this method as part of its adapter loop, so an implementation should
    return quickly and not block on the worker finishing its work.
